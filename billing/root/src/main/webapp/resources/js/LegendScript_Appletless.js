...

{
	...
	$(document).ready(function() {
		...
		$.ajax({
			type: "GET",
			url: "http://localhost:62015/MMMCSD200Service/ActionServlet"+"?action=" + strMessage + "&image=" + imageNum,
			crossDomain: true,
			dataType: 'jsonp',
			jsonp: 'callback',
			...
			success: function(jsonObj)
			{
				if(jsonObj.byteIsoImage != undefined)
				{
					...
					$( document ).ready(function() { 	
						$(".handdiv ."+button).html("<img src=\"aua/resources/image/check.jpg\"></img>");
		...
		$.ajax({
			...
			success: function(jsonObj)
			{
				if(jsonObj.byteIsoImage != undefined)
				{
					...
					//alert("document.getElementById('bfdFingerPrint').value : " + document.getElementById('bfdFingerPrint').value);
